...
def detect_baud_rate(received: np.ndarray, fs: float) -> float:
    ...
    power_spectrum = np.fft.fft(power)
    freqs = np.fft.fftfreq(N, d=1/fs)  # Hz

    pos_mask = freqs > 0
    freqs_pos = freqs[pos_mask]
    power_spectrum_pos = power_spectrum[pos_mask]

    idx_peak = np.abs(np.argmax(np.abs(power_spectrum_pos)))
    baud_est = np.abs(freqs_pos[idx_peak])
    return baud_est
